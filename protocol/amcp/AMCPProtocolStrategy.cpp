...
AMCPCommandPtr AMCPProtocolStrategy::CommandFactory(const std::wstring& str)
{
	...
	if	   (s == L"MIXER")			return std::make_shared<MixerCommand>();
	else if(s == L"DIAG")			return std::make_shared<DiagnosticsCommand>();
	else if(s == L"CHANNEL_GRID")	return std::make_shared<ChannelGridCommand>();
	else if(s == L"CALL")			return std::make_shared<CallCommand>();
	else if(s == L"SWAP")			return std::make_shared<SwapCommand>();
	else if(s == L"ROUTE")			return std::make_shared<RouteCommand>();
	else if(s == L"LOAD")			return std::make_shared<LoadCommand>();
	else if(s == L"LOADBG")			return std::make_shared<LoadbgCommand>();
	else if(s == L"ADD")			return std::make_shared<AddCommand>();
	else if(s == L"REMOVE")			return std::make_shared<RemoveCommand>();
	else if(s == L"PAUSE")			return std::make_shared<PauseCommand>();
	else if(s == L"PLAY")			return std::make_shared<PlayCommand>();
	else if(s == L"STOP")			return std::make_shared<StopCommand>();
	else if(s == L"CLEAR")			return std::make_shared<ClearCommand>();
	else if(s == L"PRINT")			return std::make_shared<PrintCommand>();
	else if(s == L"LOG")			return std::make_shared<LogCommand>();
	else if(s == L"CG")				return std::make_shared<CGCommand>();
	else if(s == L"DATA")			return std::make_shared<DataCommand>();
	else if(s == L"SIGNAL")			return std::make_shared<SignalCommand>();
	else if(s == L"CAPTURE")		return std::make_shared<CaptureCommand>();
	else if(s == L"RECORDER")		return std::make_shared<RecorderCommand>();
	else if(s == L"CINF")			return std::make_shared<CinfCommand>();
	else if(s == L"INFO")			return std::make_shared<InfoCommand>(channels_, recorders_);
	else if(s == L"CLS")			return std::make_shared<ClsCommand>();
	else if(s == L"TLS")			return std::make_shared<TlsCommand>();
	else if(s == L"VERSION")		return std::make_shared<VersionCommand>();
	else if(s == L"BYE")			return std::make_shared<ByeCommand>();
