...
NDIlib_video_frame_t * create_video_frame(const core::video_format_desc& format, const bool is_alpha, const int width, const int height)
{
	...
	if (frame)
	{
		...
		frame->line_stride_in_bytes = width * (is_alpha ? 4 : 2);
	}
	return frame;
}
