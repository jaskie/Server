...
namespace caspar { namespace ndi {
	...
	NDIlib_video_frame_t * create_weak_frame(core::video_format_desc format)
	{
...
{
	return std::shared_ptr<NDIlib_video_frame_t>(create_weak_frame(format), [](NDIlib_video_frame_t* f) 
	{
		if (f)
			delete(f->p_data);
		delete(f);
