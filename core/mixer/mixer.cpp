...
public:
		...
		{		
			try
			{
				...
				if (frames.size() == 1 && frames.at(0)->can_bypass_ogl(format_desc_.format))
				{
					...
					target_->send(std::make_pair(make_safe<read_frame>(frame, std::move(audio), audio_channel_layout_), packet.second));
	...
	safe_ptr<core::write_frame> create_frame(
			...
			const core::pixel_format_desc& pix_format,
			int timecode,
			const channel_layout& audio_channel_layout)
	{		
		return make_safe<write_frame>(ogl_, tag, format_desc_, pix_format, audio_channel_layout, timecode);
	}
...
safe_ptr<core::write_frame> mixer::create_frame(const void* tag, const core::video_format_desc& video_format, const core::pixel_format_desc& pix_format, int timecode, const channel_layout& audio_channel_layout){ return impl_->create_frame(tag, video_format, pix_format, timecode, audio_channel_layout); }
