...
namespace caspar {
	namespace core {
		...
		struct output::implementation
		{
			...
			const safe_ptr<caspar::core::signaller>			signaller_;
		...
		public:
			implementation(const safe_ptr<diagnostics::graph>& graph, const video_format_desc& format_desc, const channel_layout& audio_channel_layout, int channel_index)
				...
				, signaller_(new caspar::core::signaller(format_desc))
				...
				executor_.begin_invoke([=]
					{
						try
						{
							...
							input_frame->set_singals(signaller_->tick());
							...
							{
								auto consumer = consumers_.find(result_it->first);
								...
								catch (...)
								{
									...
									{
										CASPAR_LOG_CURRENT_EXCEPTION();
										CASPAR_LOG(error) << "Failed to recover consumer: " << consumer->second->print() << L". Removing it.";
										send_to_consumers_delays_.erase(result_it->first);
										consumers_.erase(result_it->first);
									}
								}
		...
		safe_ptr<signaller> output::signaller() const { return impl_->signaller_; }
